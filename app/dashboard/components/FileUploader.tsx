...
import { handleReadyStates } from './Elements/GithubKeys/GithubKeys';
...
*/
  ...
  const uploadFile = async (file: File, existingFileId?: string): Promise<void> => {
    ...
    let ready = await handleReadyStates(`${window.location.origin}`, `file_token`)
    if(!ready) {
        return;
    }
    ...
    if (fileData?.selectedThumbnail) {
      try {
        ...
        for (let i = 0; i < thumbnailChunks.length; i++) {
          ...
          const thumbnailResponse = await fetch(`${FileServerUrl}/upload`, {
            method: 'POST',
